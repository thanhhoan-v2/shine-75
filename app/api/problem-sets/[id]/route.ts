...
export async function DELETE(
  ...
  try {
    ...
    await sql`DELETE FROM all_problem_sets WHERE id = ${idNum}`;
...
export async function PUT(
  ...
  try {
    ...
    await sql`UPDATE all_problem_sets SET name = ${name}, description = ${description}, problems = ${JSON.stringify(problems)}, updated_at = NOW() WHERE id = ${idNum}`;
    return NextResponse.json({ success: true });
  } catch (error) {
